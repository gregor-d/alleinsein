# Cloudflare Tunnel, Caching, and Routing

This project uses Cloudflare to manage DNS, tunneling, and aggressive caching to minimize direct hits to the Hetzner VPS backend.

## Cloudflare Tunnel Setup (`tiles.alleinsein.de`)

A Cloudflare Tunnel is used to expose the backend API running on the VPS securely without opening inbound ports. The tunnel connects the local `uvicorn` instance running on `localhost:8000` to the domain `tiles.alleinsein.de`.

### Setup Instructions

1. **Create the Tunnel on Cloudflare Dashboard**:
   - Go to Zero Trust -> Networks -> Tunnels.
   - Create a new tunnel and obtain the `TUNNEL_TOKEN`.
   - Route traffic for `tiles.alleinsein.de` to `http://localhost:8000`.

2. **Run `cloudflared` on the VPS via Docker Compose**:
   Create a directory and `.env` file on the server:
   ```bash
   mkdir -p ~/cloudflare-tunnel
   cd ~/cloudflare-tunnel
   nano .env # Add: TUNNEL_TOKEN=eyJhIjoi....your-token....
   chmod 600 .env
   ```
...
## Caching and Routing Policies (`alleinsein.de`)

To ensure the backend is only hit when there is a cache miss (e.g., when a user requests a new map tile), caching is strictly configured in Cloudflare.

### Page Rules & Cache Rules
1. **Cache Everything**: Set a Cache Rule in Cloudflare for the routes serving raster tiles to **Cache Everything**.
2. **Edge Cache TTL**: Set a long Edge Cache TTL (e.g., 1 month) since map tiles are static and rarely change.
3. **Browser Cache TTL**: Configure the Browser Cache TTL to encourage clients to hold onto tiles locally.

### Request Flow
- When a user accesses `alleinsein.de`, the frontend requests tiles from `tiles.alleinsein.de`.
- Cloudflare intercepts the request:
  - **Cache Hit**: Cloudflare serves the tile directly from its edge network. The request never reaches the VPS.
  - **Cache Miss**: Cloudflare forwards the request through the secure tunnel to `localhost:8000` on the VPS. The backend generates/fetches the tile, returns it to Cloudflare, and Cloudflare caches it for future requests.
