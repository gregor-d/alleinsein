...
def process_country(country: str, args: argparse.Namespace) -> Path:
    ...
    if settings.dry_run and not (settings.bounds_dir / f"{country}.gpkg").is_file():
        settings.bbox = "0,0,0,0"  # placeholder; a real run geocodes first
    else:
        settings.bbox = bounds.get_bbox(
    ...
    pbf = settings.osm_dir / f"{country}.osm.pbf"
    if not pbf.is_file():
        osm.extract_country_pbf(europe_pbf, pbf, bounds.get_bbox_4326(settings.bbox))
    else:
    ...
    if needs_prep(args, osm_filtered):
        print(f"Filtering OSM PBF for {country}...")
        osm.filter_osm_pbf(pbf, osm_filtered)
    else:
        print(f"Reusing existing {osm_filtered} (--force-prep to rebuild)")

    if needs_prep(args, roads_gpkg):
        print(f"Building roads GeoPackage for {country}...")
        osm.create_roads_gpkg(osm_filtered, roads_gpkg)
    else:
        print(f"Reusing existing {roads_gpkg} (--force-prep to rebuild)")

    if needs_prep(args, roads_rasterized):
        print(f"Rasterizing roads for {country}...")
        osm.rasterize_roads(roads_gpkg, roads_rasterized)
    else:
        print(f"Reusing existing {roads_rasterized} (--force-prep to rebuild)")

    if needs_prep(args, roads_smooth):
        print(f"Smoothing roads for {country}...")
        osm.smooth_roads(roads_rasterized, roads_smooth)
    else:
        print(f"Reusing existing {roads_smooth} (--force-prep to rebuild)")

    # 4. CLC one-hot land-cover stack.
    if needs_prep(args, clc_stack):
...
def main() -> None:
    ...
    per_country_3035 = [
        process_country(country.lower().replace(" ", "_"), args)
        for country in settings.eu_countries
    ]
    ...
    output_cog = settings.eu_output_cog
