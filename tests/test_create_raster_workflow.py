...
def test_clip_reproject_pipeline_renders_expected_string():
    ...
    raster/create_raster.py. The block below is a copy of that code with its
    external variables turned into local inputs. Keep the two in sync: when you
    change the block in create_raster.py, mirror it here and this assertion
    proves the rendered output is unchanged."""
    ...
    # --- begin copy of the pipeline block from raster/create_raster.py ---
    pipeline = (
        dedent(
            f"""
...
def test_create_raster_dry_run_exercises_workflow():
    ...
    assert "Applying slope modifier to off-road pixels" in result.stdout
